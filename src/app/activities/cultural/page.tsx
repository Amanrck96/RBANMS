...
import { activitiesData, getActivityCategory } from '@/lib/activities'

export default function CulturalActivitiesPage() {
  const category = getActivityCategory('cultural')!

  return (
    <div className="container mx-auto px-4 py-12 md:py-16">
      <Card>
        <CardHeader>
          <CardTitle className="text-3xl md:text-4xl font-bold text-primary font-headline">Cultural</CardTitle>
        </CardHeader>
        <CardContent className="pt-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
            {category.items.map((item) => (
              <Link key={item.slug} href={`/activities/cultural/${item.slug}`} className="group">
