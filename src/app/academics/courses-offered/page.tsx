...
export default function CoursesOfferedPage() {
  return (
    <div className="min-h-screen bg-background">
      {/* Hero Section */}
      <DynamicSection
        pageId="academics-courses"
        render={(data) => {
          // If data content exists (HTML), render it.
          // If NOT (first load or no edits), render the list structure.
          // UNFORTUNATELY, mixing "HTML" content and "Structured JSON" is hard in this generic CMS.
          // STRATEGY: We will render the "Page Header" via data.title/tagline
          // And we will fallback to the hardcoded list if data.content is empty. 
          // BUT, if the user edits the content box, we will effectively REPLACE the list with their HTML.
          // This gives them full control to rewrite the page if they want, OR keep the list.

          return (
            <>
              <div className="bg-white py-16">
                <div className="container mx-auto px-4 text-center">
                  <Badge variant="secondary" className="mb-4 text-primary font-bold bg-blue-50">{data.badgeText || "Academic Excellence"}</Badge>
                  <h1 className="text-4xl md:text-5xl font-bold font-headline mb-4 text-primary">{data.title || "Courses Offered"}</h1>
                  <p className="text-lg md:text-xl max-w-2xl mx-auto text-muted-foreground">
                    {data.tagline || "Discover our diverse range of undergraduate programs designed to empower you with knowledge, skills, and future-ready capabilities."}
                  </p>
                </div>
              </div>
